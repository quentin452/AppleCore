...
public class CommandHunger extends CommandBase {
    ...
    public void processCommand(ICommandSender commandSender, String[] args) {
        if (args.length > 0) {
            EntityPlayerMP playerToActOn = args.length >= 2 ? getPlayer(commandSender, args[1])
                    : getCommandSenderAsPlayer(commandSender);
            int newHunger = args.length >= 2 ? parseIntBounded(commandSender, args[1], 0, 20)
            ...
            func_152374_a(
                    ...
                    StatCollector.translateToLocalFormatted(
                            "applecore.commands.hunger.set.hunger.to",
                            playerToActOn.getDisplayName(),
                            newHunger));
        } else {
